...
namespace Deployer.Lumia.Gui.ViewModels
{
    ...
    public class DeploymentViewModel : ReactiveObject, ISection
    {
        ...
        private async Task Deploy()
        {
            ...
            var preparerName = lumiaSettingsService.DiskPreparer ?? string.Empty;
            var isWipe = preparerName.IndexOf("Wipe", StringComparison.OrdinalIgnoreCase) >= 0;
            var confirm = await uiServices.ContextDialog.ShowYesNo(this,
                isWipe ? "Wipe Windows 10 Mobile?" : "Deploy Windows?",
                isWipe
                    ? "Windows 10 Mobile (MainOS and Data) will be deleted.\n\nCharge to 100%. Remove the microSD. Unplug other USB disks.\n\nThis cannot be undone. Continue?"
                    : "Windows 10 Mobile will be kept (dual-boot layout).\n\nCharge to 100%. Remove the microSD. In Disk Management the phone disk must be Online.\n\nContinue?",
                isYesDefault: !isWipe);

            if (confirm != DialogResult.Yes)
            {
                Log.Information("Deployment cancelled by user");
                return;
            }
