...
namespace Deployer.Lumia
{
    public class Phone : Device, IPhone
    {
        ...
        private static readonly ByteSize MinimumPhoneDiskSize = ByteSize.FromGigaBytes(16);
        private static readonly ByteSize MaximumPhoneDiskSize = ByteSize.FromGigaBytes(128);
        ...
        private async Task<IDisk> GetDeviceDiskCore()
        {
            ...
            var matches = new List<IDisk>();
            foreach (var candidate in disks)
            {
                if (await IsDeviceDisk(candidate))
                {
            ...
            if (matches.Count > 1)
            {
                throw new PhoneDiskNotFoundException(
                    $"Multiple disks look like a Lumia ({matches.Count}). Unplug extra USB drives so the phone is the only Mass Storage disk, then retry.");
            }
        ...
        private static async Task<bool> IsDeviceDisk(IDisk disk)
        {
            ...
            var uniqueId = disk.UniqueId ?? string.Empty;
            var hasCorrectDiskName = diskNames.Any(name => uniqueId.IndexOf(name, StringComparison.OrdinalIgnoreCase) >= 0);
