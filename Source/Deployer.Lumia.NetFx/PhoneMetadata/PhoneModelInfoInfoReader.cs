...
namespace Deployer.Lumia.NetFx.PhoneMetadata
{
    public class PhoneModelInfoInfoReader : IPhoneModelInfoReader
    {
        ...
        private static Variant GetVariant(PhoneInfo info)
        {
            ...
            if (!dict.TryGetValue(info.Ddp.PhoneType, out var variant))
            {
                throw new InvalidOperationException(
                    $"Unknown phone type '{info.Ddp.PhoneType}'. WOA Deployer supports Lumia 950 (RM-1104/1105) and Lumia 950 XL (RM-1085/1116/1118).");
            }
        ...
        private static PhoneModel PhoneModel(PhoneInfo info)
        {
            ...
            if (!dict.TryGetValue(key, out var model))
            {
                throw new InvalidOperationException(
                    $"Unknown platform '{name}' / RKH '{rkhStr}'. This phone is not a supported Lumia 950 / 950 XL.");
            }
