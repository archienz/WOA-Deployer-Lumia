...
namespace Deployer.Lumia.Gui.ViewModels
{
    ...
    public class AdvancedViewModel : ReactiveObject, ISection, IDisposable
    {
        ...
        {
            var confirm = await uiServices.ContextDialog.ShowYesNo(this, "Force Dual Boot?",
                "Use only if dual-boot status is wrong after a successful WOA install.\n\nDo not use during driver injection or first setup. Safety checks are skipped.\n\nContinue?",
                isYesDefault: false);
        ...
        {
            var confirm = await uiServices.ContextDialog.ShowYesNo(this, "Force Single Boot?",
                "This skips dual-boot capability checks and rewrites BCD. Only use after a successful WOA install.\n\nContinue?",
                isYesDefault: false);
            if (confirm != DialogResult.Yes)
            {
                return;
